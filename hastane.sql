    ...
    CREATE TABLE "iller"."Oda" (
        ...
        "v_kapasite" INT DEFAULT 1,

        CONSTRAINT "odaPK" PRIMARY KEY ("hastaneId", "odaNo"), -- bir hastanede oda numarası tekrar edemez
        
        CONSTRAINT "kapasiteKontrol" CHECK ("v_kapasite" >= 1), -- kapasite en az 1 olmalı
    ...
    RETURNS TABLE (
        ...
        SELECT 
            ...
            ''Muayene''::TEXT AS "IslemTuru",
            COALESCE(m."tani", ''Tanı Girilmedi'')::TEXT AS "Aciklama"
        FROM "vatandas"."Vatandas" v
        JOIN "randevular"."Randevu" r ON v."vatandasId" = r."hastaId"
        LEFT JOIN "randevular"."Muayene" m ON r."randevuId" = m."randevuId"
        WHERE v."tcKimlikNo" = p_tc

        UNION ALL

        SELECT
            y."yatisTarihi" AS "Tarih",
            ''Yatış İşlemi''::TEXT AS "IslemTuru",
            (''Hastane: '' || h."hastaneAdi" || '' - Oda: '' || y."odaNo" || 
             CASE 
                WHEN y."taburcuTarihi" IS NULL THEN '' (Halen Yatıyor)'' 
                ELSE '' (Taburcu: '' || y."taburcuTarihi" || '')'' 
             END)::TEXT AS "Aciklama"
        FROM "vatandas"."Vatandas" v
        JOIN "randevular"."Yatis" y ON v."vatandasId" = y."hastaId"
        JOIN "iller"."Hastane" h ON y."hastaneId" = h."hastaneId"
        WHERE v."tcKimlikNo" = p_tc
    ...
    DECLARE 
        hasta_zaten_burada INT;
        mevcut_doluluk INT; 
        oda_siniri INT; 
    BEGIN
        SELECT COUNT(*) INTO hasta_zaten_burada 
        FROM "randevular"."Yatis"
        WHERE "hastaId" = NEW."hastaId" 
          AND "hastaneId" = NEW."hastaneId" 
          AND "odaNo" = NEW."odaNo" 
          AND "taburcuTarihi" IS NULL;

        IF hasta_zaten_burada > 0 THEN
            RAISE EXCEPTION ''HATA: Bu hasta zaten seçilen odada yatmaktadır!'';
        END IF;
        
        SELECT "v_kapasite" INTO oda_siniri 
        FROM "iller"."Oda" 
        WHERE "hastaneId" = NEW."hastaneId" AND "odaNo" = NEW."odaNo";
        
        SELECT COUNT(*) INTO mevcut_doluluk 
        FROM "randevular"."Yatis" 
        WHERE "hastaneId" = NEW."hastaneId" 
          AND "odaNo" = NEW."odaNo" 
          AND "taburcuTarihi" IS NULL;

        IF mevcut_doluluk >= oda_siniri THEN 
            RAISE EXCEPTION ''HATA: Seçilen oda dolu! Kapasite aşıldı.''; 
        END IF;
    ...
    CREATE TRIGGER "trg_OdaKapasite"
    BEFORE INSERT ON "randevular"."Yatis"
    FOR EACH ROW
    EXECUTE FUNCTION "randevular"."trg_OdaKapasite_Func"();
    ...
    CREATE TRIGGER "trg_RandevuTarih"
    BEFORE INSERT ON "randevular"."Randevu"
    FOR EACH ROW
    EXECUTE FUNCTION "randevular"."trg_RandevuTarih_Func"();
    ...
    AFTER DELETE ON "randevular"."Randevu"
    FOR EACH ROW
    EXECUTE FUNCTION "randevular"."trg_RandevuLog_Func"();
    ...
    CREATE TRIGGER "trg_ReceteAdet"
    BEFORE INSERT OR UPDATE ON "randevular"."ReceteDetay"
    FOR EACH ROW
    EXECUTE FUNCTION "randevular"."trg_ReceteAdet_Func"();
    ...
    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (7, 77777777777, 'Hilal', 'Koç', 'Keçiören/Ankara');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (7, 'Doktor', 'Prof. Dr.', 'DR-103', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (7, 'Kardiyoloji');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (7, 7, 1);

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (8, 88888888888, 'Murat', 'Aslan', 'Çankaya/Ankara');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (8, 'Doktor', 'Uzm. Dr.', 'DR-104', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (8, 'Kardiyoloji');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (8, 8, 1);

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (9, 99999999990, 'Ayşe', 'Çelik', 'Mamak/Ankara');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (9, 'Doktor', 'Yrd. Doç.', 'DR-105', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (9, 'Dahiliye');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (9, 9, 1);

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (10, 99999999992, 'Fatma', 'Demir', 'Etimesgut/Ankara');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (10, 'Doktor', 'Op. Dr.', 'DR-106', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (10, 'Kardiyoloji');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (10, 10, 1);

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (11, 10000000011, 'Kenan', 'Çakır', 'Çankaya/Ankara');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (11, 'Doktor', 'Prof. Dr.', 'DR-107', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (11, 'Dahiliye');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (11, 11, 1); 

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (12, 10000000012, 'Banu', 'İkiz', 'Mamak/Ankara');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (12, 'Doktor', 'Uzm. Dr.', 'DR-108', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (12, 'Dahiliye');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (12, 12, 1); 

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (13, 10000000013, 'Vedat', 'Gökhan', 'Beylikdüzü/İstanbul');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (13, 'Doktor', 'Op. Dr.', 'DR-203', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (13, 'Göz Hastalıkları');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (13, 13, 2);

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (14, 10000000014, 'Emine', 'Sayan', 'Fatih/İstanbul');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (14, 'Doktor', 'Prof. Dr.', 'DR-204', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (14, 'Göz Hastalıkları');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (14, 14, 2);

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (15, 10000000015, 'Sinan', 'Öztürk', 'Başakşehir/İstanbul');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (15, 'Doktor', 'Op. Dr.', 'DR-205', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (15, 'Kulak Burun Boğaz');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (15, 15, 2); 

    INSERT INTO "vatandas"."Vatandas" ("vatandasId", "tcKimlikNo", "ad", "soyad", "adres") VALUES (16, 10000000016, 'Melis', 'Şanslı', 'Fatih/İstanbul');
    INSERT INTO "vatandas"."Personel" ("personelId", "gorevTuru", "unvan", "sicilNo", "sifre") VALUES (16, 'Doktor', 'Uzm. Dr.', 'DR-206', '1234');
    INSERT INTO "vatandas"."Doktor" ("doktorId", "uzmanlikAlani") VALUES (16, 'Kulak Burun Boğaz');
    INSERT INTO "vatandas"."HastanePersonel" ("hastanePersonelId", "personelId", "hastaneId") VALUES (16, 16, 2); 

    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (1, '101', 1);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (1, '102', 2);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (1, '103', 3);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (1, '104', 3);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (1, '105', 4);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (2, '201', 1);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (2, '202', 4);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (2, '203', 3);
    INSERT INTO "iller"."Oda" ("hastaneId", "odaNo", "v_kapasite") VALUES (2, '204', 2);
